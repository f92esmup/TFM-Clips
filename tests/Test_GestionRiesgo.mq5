...
void OnStart()
  {
   ...
   CRiskManager riesgo(20, 30, 10, 1.0, 0.5);
   ...
   params = riesgo.GetTradeParams(balance);
   // Riesgo Base (1.0% de 10000 = 100.0)
   Check("3. Consenso 25 activa Nivel 1 (1.0%)", params.risk_amount == 100.0);

   // Test 4: Nivel 2 Alcanzado (T2=50)
   for(int i = 25; i < 60; i++) riesgo.UpdateAgentVote(i, "comprar");
   params = riesgo.GetTradeParams(balance);
   // Nivel 2 (1.0% + 0.5% = 1.5% de 10000 = 150.0)
   Check("4. Consenso 60 activa Nivel 2 (1.5%)", params.risk_amount == 150.0);

   // Test 5: Nivel 3 Alcanzado (T3=80)
   for(int i = 60; i < 90; i++) riesgo.UpdateAgentVote(i, "comprar");
   params = riesgo.GetTradeParams(balance);
   // Nivel 3 (1.0% + 1.0% = 2.0% de 10000 = 200.0)
   Check("5. Consenso 90 activa Nivel 3 (2.0%)", params.risk_amount == 200.0);
   ...
   params = riesgo.GetTradeParams(balance);
   Check("6. Votos bajistas cancelan alcistas (Neto 40 cae a Nivel 1)", params.risk_amount == 100.0);
   ...
   params = riesgo.GetTradeParams(balance);
   Check("7. Histéresis mantiene Nivel 2 al caer a 45 (Tolerancia 40)", params.risk_amount == 150.0);
